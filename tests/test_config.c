...
int main(void) {
    ...
    /* naming rule: section/key names are [a-z][a-z0-9_]* — must start with a
     * lowercase letter; uppercase, spaces, hyphens, or a leading digit/underscore
     * are a parse error (prov convention). */
    {
        prov_config_result_t e1 = parse(&c, "[Editor]\ntabstop = 8\n");
        CHECK(!e1.ok, "uppercase section name rejected");
        prov_config_result_t e2 = parse(&c, "[ed itor]\n");
        CHECK(!e2.ok, "space in section name rejected");
        prov_config_result_t e3 = parse(&c, "[editor]\nTabStop = 8\n");
        CHECK(!e3.ok, "uppercase key name rejected");
        prov_config_result_t e4 = parse(&c, "[my-section]\n");
        CHECK(!e4.ok, "hyphen in section name rejected");
        prov_config_result_t e5 = parse(&c, "[2things]\n");
        CHECK(!e5.ok, "leading-digit section name rejected");
        prov_config_result_t e6 = parse(&c, "[editor]\n_key = 1\n");
        CHECK(!e6.ok, "leading-underscore key name rejected");
        prov_config_result_t okp = parse(&c, "[my_section_2]\nsome_key3 = \"x\"\n");
        CHECK(okp.ok, "lowercase-start letter/digit/underscore names accepted");
    }
