...
/* Write the user's home directory (no trailing slash, UTF-8) into `out`.
 * POSIX: $HOME. Windows: %USERPROFILE%, else %HOMEDRIVE%%HOMEPATH%, else %APPDATA%.
 * Used to resolve ~/.prov for the config and theme files. Returns false if none
 * is set. */
bool prov_platform_home_dir(char *out, proven_size_t cap);

/* A mounted drive/volume (Windows). */
typedef struct {
    char               letter;     /* 'A'..'Z' */
    char               label[64];  /* volume label (UTF-8), "" if none */
    unsigned long long total;      /* bytes, 0 if unknown */
    unsigned long long avail;      /* free bytes available, 0 if unknown */
} prov_drive_t;

/* List mounted drives into `out` (up to `cap`). Returns the count. POSIX has no
 * drive letters, so it returns 0 (the editor then shows the filesystem root). */
