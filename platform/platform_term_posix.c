...
void prov_term_present(const prov_cell_t *grid, proven_size_t rows,
    ...
    for (proven_size_t r = 0; r < rows; r++) {
        ...
        int fg = 0;                              /* biased: 0 = default fg (see display.h) */
        for (proven_size_t c = 0; c < cols; c++) {
            ...
            int  want_fg   = (int)cell.fg;       /* 0 = default, 1..16 = ANSI+1 */
            ...
            if (want_fg != fg) {                 /* syntax-highlight foreground (RFC-0022) */
                if (want_fg == 0) ob_str("\x1b[39m");          /* back to default */
                else {
                    int idx = want_fg - 1;                     /* 0..15 ANSI */
                    char seq[8];
                    int code = idx < 8 ? 30 + idx : 90 + (idx - 8);
                    int sl2 = 0; seq[sl2++] = '\x1b'; seq[sl2++] = '[';
        ...
        if (fg)  ob_str("\x1b[39m");             /* leave non-default fg */
    ...
    /* command line (normal video) below the status bar. The byte \x01 toggles
     * shortcut emphasis (bold+underline) for legends; it is not printed. */
    ...
    for (proven_size_t i = 0, col = 0; i < cl && col < cols; i++) {
        ...
        ob_put(&ch, 1);
        if ((ch & 0xC0) != 0x80) col++;          /* count UTF-8 lead bytes as columns */
    }
...
int prov_platform_list_drives(prov_drive_t *out, int cap) {
    (void)out; (void)cap;     /* POSIX has a single rooted filesystem, no drives */
    return 0;
}
