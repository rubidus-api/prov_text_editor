 ...
 *
 * Naming rule (prov convention): section names and key names must start with a
 * lowercase ASCII letter and otherwise use only lowercase letters, digits and
 * underscore — [a-z][a-z0-9_]*. No uppercase, spaces, hyphens, or leading
 * digit/underscore. A name breaking this rule is a parse error.
 */
...
typedef struct {
    ...
    char       theme[32];          /* [ui] theme          — color theme name (RFC-0021); "" = prov-dark */
} prov_config_t;
