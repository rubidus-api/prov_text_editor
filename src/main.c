...
typedef struct prov_session {
    ...
    prov_theme_t       theme;           /* resolved color theme (RFC-0021; loaded, not yet rendered) */
    ...
    struct {
        ...
        bool                 save;             /* zw write mode: pick a target + filename to WRITE
                                                * (no preview / no hex / no read-only; has discard) */
        int                  save_buf;         /* buffer being written in save mode */
    } browse;
...
enum { BF_LIST = 0, BF_PREVIEW, BF_PATH, BF_ENC, BF_BACKEND, BF_BOM, BF_EOL, BF_RO, BF_COUNT };  /* browser Tab focus (RFC-0015) */
...
static void panel_open_write(prov_session_t *s, int bufidx);    /* zw: browser-style write panel */
static void browse_write_to(prov_session_t *s, const char *path);   /* zw commit */
    ...
    prov_editor_t *ed = s->ed;
            switch (cmd.edit) {
                case PROV_EDIT_UNDO:
                    ...
                    s->modified = true;
                    break;
                case PROV_EDIT_REDO:                          /* U: redo ([N]U redoes N steps) */
                    if (ro_guard(s)) break;
    ...
    const prov_buffer_t *b = prov_editor_buffer(ed);
            switch (cmd.action) {
                case PROV_ACT_RETURN_ED:        /* zx: plain return to Ed mode (no insertion) */
                    ...
                    break;
                case PROV_ACT_WRITE_FILE:                       /* zw: the write panel (browser-style) */
                    panel_open_write(s, active_buf(s));
                    break;
                ...
                case PROV_ACT_REPLACE:                                                  /* sr */
                    ...
                    break;
                case PROV_ACT_SEARCH_HELP: panel_open_help(s, 'X'); break;              /* sh: regex reference */
...
static bool config_path(proven_allocator_t a, char *out, proven_size_t cap,
    ...
    char exedir[1024];
    bool have_exe = prov_platform_exe_dir(exedir, sizeof exedir);
    if (have_exe) {
        char p[1100];
    ...
    /* Home copy: $HOME (POSIX) or %USERPROFILE%/%HOMEDRIVE%%HOMEPATH%/%APPDATA%
     * (Windows), resolved as UTF-8 by the PAL. */
    char home[1024];
    if (prov_platform_home_dir(home, sizeof home)) {
        char hp[1100];
    ...
    /* No home environment (e.g. a bare Windows shell): fall back to a portable
     * config created next to the executable. home_dir stays "" so open_config
     * skips the mkdir (the exe directory already exists). */
    if (have_exe) {
        char p[1100];
        FMT_INTO(p, "{}/provconf.ini", PROVEN_ARG(prov_cstr_view(exedir)));
        prov_cstr_set(out, cap, prov_cstr_view(p));
...
/* The directory holding the resolved config file (and the *.theme.ini files):
 * config_path's path with the file name stripped. */
static bool config_dir(proven_allocator_t a, char *out, proven_size_t cap) {
    char path[1100];
    if (!config_path(a, path, sizeof path, NULL, 0)) return false;
    char *slash = NULL;
    for (char *p = path; *p; p++) if (*p == '/' || *p == '\\') slash = p;
    if (!slash) return false;
    *slash = '\0';
    prov_cstr_set(out, cap, prov_cstr_view(path));
    return true;
}
...
static void maybe_reapply_config(prov_session_t *s, prov_editor_t *ed, const char *path) {
    ...
    config_apply(s, &nc);
    {   /* re-resolve the color theme on live config apply (RFC-0021) */
        char cdir[1100];
        s->theme = prov_theme_resolve(s->a, config_dir(s->a, cdir, sizeof cdir) ? cdir : NULL, s->cfg.theme);
    }
...
static void browser_vrow(void *ctx, proven_size_t i, char *buf, proven_size_t cap) {
    ...
    if (s->browse.model.dir[0] == '\0') {            /* drives screen: "  C:\   <label>  free/total" */
        proven_u8str_t b = proven_u8str_borrow((proven_byte_t *)buf, cap);
        (void)proven_u8str_append_fmt_trunc(&b, "  {}\\   {}",
            PROVEN_ARG(prov_cstr_view(e->name)), PROVEN_ARG(prov_cstr_view(e->label)));
        (void)proven_u8str_as_cstr(&b);
        return;
    }
...
static void panel_open_browser(prov_session_t *s) {
    ...
    s->browse.save = false;                       /* zo: open mode */
...
/* zw write panel: same browser, but writes the active buffer. Modeled on zo, with
 * a discard (d), and with preview / hex / read-only removed (irrelevant to saving).
 * The path field is seeded with the current file name; Enter / o writes it. */
static void panel_open_write(prov_session_t *s, int bufidx) {
    panel_open_browser(s);                         /* reuse the whole browser setup */
    s->browse.save = true;
    s->browse.save_buf = bufidx;
    s->browse.preview = false;                     /* no preview in write mode */
    s->browse.focus = BF_PATH;                     /* start in the filename field */
    s->open_binary = false; s->open_ro = false;    /* hex / read-only do not apply to writing */
    prov_le_clear(&s->browse.pathedit);
    const char *cur = s->bufs.entries[bufidx].path;
    if (cur && cur[0]) prov_le_set(&s->browse.pathedit, prov_basename(cur));  /* seed the file name */
    FMT_INTO(s->message, "write to: pick a folder, type a name, Enter to save");
}

/* zw commit: write the save-mode buffer to `path` (current dir + the typed name),
 * applying the panel's BOM / EOL options. Closes the panel on success. */
static void browse_write_to(prov_session_t *s, const char *path) {
    int b = s->browse.save_buf;
    if (b < 0 || b >= s->bufs.count) { panel_close(s); return; }
    prov_fileinfo_t *info = &s->bufs.entries[b].info;
    if (s->open_bom) info->bom = true;                         /* zw BOM option */
    if (s->open_eol > 0) info->eol = (prov_eol_t)(s->open_eol - 1);   /* zw EOL option */
    proven_err_t err = prov_save_buffer(s->a, prov_editor_buffer(s->bufs.entries[b].ed), path, info);
    if (!PROVEN_IS_OK(err)) {
        FMT_INTO(s->message, "could not write \"{}\" — check the folder and permissions",
                 PROVEN_ARG(prov_cstr_view(path)));
        return;
    }
    prov_cstr_set(s->bufs.entries[b].path, sizeof s->bufs.entries[b].path, prov_cstr_view(path));  /* now named */
    if (b == active_buf(s)) {
        prov_editor_compact(s->ed); s->modified = false;
        s->path = s->bufs.entries[b].path;
    } else s->bufs.entries[b].modified = false;
    FMT_INTO(s->message, "saved {}", PROVEN_ARG(prov_cstr_view(path)));
    panel_close(s);
    maybe_reapply_config(s, s->bufs.entries[b].ed, path);
}

/* `I`: ascend to the parent directory (entry 0 = ".."). */
static void browser_parent(prov_session_t *s) {
    if (s->browse.model.dir[0] == '\0') return;       /* drives screen is the top */
    char up[1024];
...
static bool pa_browser(prov_session_t *s, int id) {
    ...
    if (s->browse.save) {                          /* zw: overwrite the selected file */
        browse_write_to(s, path);
        return false;                              /* browse_write_to closes on success */
    }
...
static void browser_path_submit(prov_session_t *s) {
    ...
    if (s->browse.save) {                          /* zw: write to the typed path (may be new) */
        proven_fs_stat_t wst;
        if (PROVEN_IS_OK(proven_fs_stat(s->a, prov_cstr_view(resolved), &wst))
            && wst.type == PROVEN_FS_TYPE_DIR) {   /* a directory: descend, keep choosing */
            s->browse.focus = BF_LIST; prov_le_clear(&s->browse.pathedit); browser_goto(s, resolved); return;
        }
        browse_write_to(s, resolved);              /* file (new or existing): write it */
        return;
    }
    proven_fs_stat_t st;
...
static void panel_key(prov_session_t *s, prov_key_t k) {
    ...
    if (s->panel_kind == PANEL_K_BROWSER && k.kind == PROV_KEY_TAB) {
        ...
        do { f = (f + step) % BF_COUNT;                   /* skip preview (off) and, in write mode, read-only */
        } while ((f == BF_PREVIEW && !s->browse.preview) || (f == BF_RO && s->browse.save));
        s->browse.focus = f;
        return;
    }
    ...
    if (s->panel_kind == PANEL_K_BROWSER && s->browse.focus >= BF_ENC
        ...
        switch (s->browse.focus) {                        /* Enter/Space activates the Tab-focused option */
            ...
            case BF_EOL:     browse_opt_eol(s);     break;
    ...
    switch (r.kind) {
        ...
        case PK_VERB:
            if (s->panel_kind == PANEL_K_BROWSER) {
                switch (r.action) {                        /* browser verbs are immediate (no slot) */
                    ...
                    case PV_RO:      if (!s->browse.save) browse_opt_ro(s);     break;  /* R: read-only (open only) */
                    case PV_BINARY:  if (!s->browse.save) browse_opt_binary(s); break;  /* x: hex (open only) */
                    ...
                    case PV_PREVIEW:                                   /* v: toggle the preview pane (open only) */
                                    if (s->browse.save) break;
        ...
        case PK_DISCARD:
            if (s->panel_kind == PANEL_K_BROWSER && s->browse.save) {   /* zw: reset name + options */
                s->open_bom = false; s->open_eol = 0;
                prov_le_clear(&s->browse.pathedit);
                FMT_INTO(s->message, "write options reset");
            }
            /* zo (open) browser: no discard — `d` is inert there */
            break;
...
static void search_run(prov_session_t *s, const proven_u8 *hay, proven_size_t haylen,
    ...
    if (s->search.regex) {                            /* regex path (RFC-0009) */
        ...
        if (found) {
            /* select the current match so it stands out from the yellow highlights;
             * anchor=end, cursor=start keeps the cursor at the match start (so
             * next/prev advance exactly as before). */
            if (m.end > m.start) prov_editor_select_range(s->ed, m.end, m.start);
            else prov_editor_move_to(s->ed, m.start);
    ...
    if (found) {
        /* select the current match (distinct from the yellow highlights); anchor=end,
         * cursor=start so the cursor sits at the match start as before. */
        if (nlen > 0) prov_editor_select_range(s->ed, pos + nlen, pos);
        else prov_editor_move_to(s->ed, pos);
        FMT_INTO(s->message, "{}{}", PROVEN_ARG(prov_cstr_view(forward ? "/" : "?")),
                 PROVEN_ARG(prov_cstr_view(s->search.term)));
    } else {
        FMT_INTO(s->message, "not found: {}", PROVEN_ARG(prov_cstr_view(s->search.term)));
...
static void draw_panel(prov_cell_t *grid, int cols, int rows, prov_session_t *s) {
    ...
    int ctop = r0 + 1, ch = (br - 1) - ctop + 1;       /* content rows: ctop .. br-1
                                                        * (the legend now lives on the
                                                        * global command line). */
    if (ch < 1) ch = 1;
    ...
    for (int r = ctop; r <= br - 1; r++) grid[r * cols + c0]   = (prov_cell_t){ .cp = 0x2502 };  /* left │ */
    ...
    if (s->panel_kind == PANEL_K_HEXEDIT && s->hexedit.ed) {   /* RFC-0019 P3: multi-line text edit */
        prov_editor_t *te = s->hexedit.ed;
        int hch = (br - 1) - ctop + 1; if (hch < 1) hch = 1;
        ...
        FMT_INTO(l1, "  {}{}{}", PROVEN_ARG(prov_cstr_view(ph)),
                 PROVEN_ARG(prov_cstr_view(s->saveas_state == SA_PATH ? "\xe2\x96\x8f" : "")),
        ...
        {   /* options row: x/w/c keyed; highlight toggled by focus (h = help) */
            ...
            int ocol = 0;
            for (int oi = 0; oi < nopt && row <= br - 1; oi++) {
                char seg[40];
        ...
        for (int r = ctop; r <= br - 1; r++)                          /* right border (scrollbars overwrite) */
            grid[r * cols + c0 + w - 1] = (prov_cell_t){ .cp = 0x2502 };
        int opt_row = br - 1, path_row = br - 2, sep1_row = br - 3;   /* fixed bottom block */
        ...
        {   /* options row: read-options shown side by side. Each segment is
             *   "<key> label:value"  with the key letter emphasized (bold+underline);
             * the Tab-focused segment is reversed. (sort has no Tab focus: foc=-1.) */
            char sortv[24];
            FMT_INTO(sortv, "{}{}",
                PROVEN_ARG(prov_cstr_view(s->browse.sort == BSORT_MTIME ? "date"
                                        : s->browse.sort == BSORT_EXT   ? "ext" : "name")),
                PROVEN_ARG(prov_cstr_view(s->browse.sort_desc ? "\xe2\x86\x93" : "\xe2\x86\x91")));  /* ↓ / ↑ */
            struct { char key; const char *label; const char *val; int foc; } opt[] = {
                { 't', "sort",      sortv,                                                       -1 },
                { 'e', "enc",       s->open_enc[0] ? s->open_enc : "UTF-8(auto)",                BF_ENC },
                { 'm', "backend",   s->cfg.charset_backend[0] ? s->cfg.charset_backend : "auto", BF_BACKEND },
                { 'b', "BOM",       s->open_bom ? "on" : "off",                                   BF_BOM },
                { 'r', "EOL",       eol_choice_name(s->open_eol),                                 BF_EOL },
                { 'R', "Read-only", s->open_ro ? "on" : "off",                                    BF_RO },
            };
            int nopt = s->browse.save ? 5 : 6;     /* zw write: no read-only option */
            int ocol = 0;
            for (int oi = 0; oi < nopt; oi++) {
                char seg[96];
                char kbuf[2] = { opt[oi].key, '\0' };
                FMT_INTO(seg, "{} {}:{}", PROVEN_ARG(prov_cstr_view(kbuf)),
                         PROVEN_ARG(prov_cstr_view(opt[oi].label)), PROVEN_ARG(prov_cstr_view(opt[oi].val)));
                int segw = (int)prov_str_disp_width(seg);
                if (ocol + segw > iw - 1) break;
                ...
                /* emphasize the leading key letter */
     ...
     * The key legend is no longer drawn inside the box — it is shown on the global
     * command line (build_cmdline) for layout uniformity. */
...
/* Build a syntax-highlight foreground map (RFC-0022) for the visible logical lines
 * of `ed`. Returns the biased-fg buffer (document-byte indexed from *fg_base) and
 * its length via *fg_len, or NULL when nothing should be highlighted (no theme
 * color, unknown language, binary buffer, or the span exceeds the cap → plain).
 *
 * A per-buffer line-state cache (keyed by editor + buffer revision + language)
 * stores the carry state AFTER each line, so unedited frames (scrolling, idle
 * redraws) start from the cached state with no rescan; after an edit the cache is
 * invalidated and the start state is re-seeded from a bounded anchor above the
 * viewport (correct, self-correcting). */
#define HL_FGCAP   (96u * 1024u)   /* max colored span per frame (else plain) */
#define HL_LINECAP 8192u           /* max bytes scanned per line */
#define HL_LOOKBACK 600u           /* lines scanned above the viewport when cold */
static proven_u8 g_hl_fg[HL_FGCAP];
static proven_u8 g_hl_line[HL_LINECAP];
static proven_u8 g_hl_scratch[HL_LINECAP];   /* discarded fg for state-only scans */

/* line-state cache: ends[L] = carry state AFTER line L; valid for lines < n_valid */
static struct {
    const prov_editor_t *ed;
    proven_size_t        revision;
    prov_hl_lang_t       lang;
    prov_hl_state_t     *ends;
    proven_size_t        cap;       /* allocated entries */
    proven_size_t        n_valid;   /* ends[0..n_valid) are valid */
} g_hl_cache;

/* state at the START of line L from the cache (STATE0 for line 0 / cold). */
static prov_hl_state_t hl_cache_start(proven_size_t L) {
    if (L == 0) return PROV_HL_STATE0;
    if (L - 1 < g_hl_cache.n_valid) return g_hl_cache.ends[L - 1];
    return PROV_HL_STATE0;
}

/* read line L's bytes into `buf` (capped), return the count. */
static proven_size_t hl_read_line(const prov_buffer_t *b, proven_size_t L,
                                  proven_size_t lc, proven_u8 *buf) {
    proven_size_t ls = prov_buffer_line_start(b, L);
    proven_size_t le = (L + 1 < lc) ? prov_buffer_line_start(b, L + 1) - 1
                                    : prov_buffer_byte_len(b);
    proven_size_t n = le - ls; if (n > HL_LINECAP) n = HL_LINECAP;
    prov_buffer_copy_range(b, ls, n, buf, n);
    return n;
}

static const proven_u8 *build_highlight(prov_session_t *s, const prov_editor_t *ed,
                                        proven_size_t top, proven_size_t rows,
                                        proven_size_t *fg_base, proven_size_t *fg_len) {
    int bi = bufset_index_of(s, ed);
    if (bi < 0) return NULL;
    if (s->bufs.entries[bi].info.binary) return NULL;           /* hex/binary: no highlight */
    prov_hl_lang_t lang = prov_hl_detect(s->bufs.entries[bi].path);
    if (lang == PROV_HL_NONE) return NULL;

    const prov_buffer_t *b = prov_editor_buffer(ed);
    proven_size_t lc = prov_buffer_line_count(b);
    if (lc == 0) return NULL;
    proven_size_t rev = prov_buffer_revision(b);

    proven_size_t vis_first = top;
    proven_size_t vis_last  = top + rows; if (vis_last > lc) vis_last = lc;
    if (vis_first >= vis_last) return NULL;

    proven_size_t base = prov_buffer_line_start(b, vis_first);
    proven_size_t span_end = (vis_last < lc) ? prov_buffer_line_start(b, vis_last)
                                             : prov_buffer_byte_len(b);
    if (span_end < base || span_end - base > HL_FGCAP) return NULL;   /* too big: plain */
    proven_size_t span = span_end - base;
    for (proven_size_t i = 0; i < span; i++) g_hl_fg[i] = 0;

    /* (re)validate the cache: any change of buffer/revision/language drops it */
    if (g_hl_cache.ed != ed || g_hl_cache.revision != rev || g_hl_cache.lang != lang) {
        if (g_hl_cache.cap < lc) {                 /* grow the ends array to fit */
            proven_size_t ncap = lc;
            proven_result_mem_mut_t r = g_hl_cache.ends
                ? s->a.realloc_fn(s->a.ctx, g_hl_cache.ends,
                                  g_hl_cache.cap * sizeof(prov_hl_state_t),
                                  ncap * sizeof(prov_hl_state_t), 8)
                : s->a.alloc_fn(s->a.ctx, ncap * sizeof(prov_hl_state_t), 8);
            if (PROVEN_IS_OK(r.err)) { g_hl_cache.ends = (prov_hl_state_t *)r.value.ptr; g_hl_cache.cap = ncap; }
        }
        g_hl_cache.ed = ed; g_hl_cache.revision = rev; g_hl_cache.lang = lang;
        g_hl_cache.n_valid = 0;
    }
    bool have_cache = g_hl_cache.ends && g_hl_cache.cap >= lc;

    /* choose where to start the downward scan that establishes the state at
     * vis_first: the cached frontier if it reaches the viewport, else a bounded
     * anchor above it (cold). */
    proven_size_t start;
    if (have_cache && g_hl_cache.n_valid >= vis_first) start = vis_first;       /* cache hit */
    else {
        proven_size_t anchor = (vis_first > HL_LOOKBACK) ? vis_first - HL_LOOKBACK : 0;
        start = (have_cache && g_hl_cache.n_valid > anchor) ? g_hl_cache.n_valid : anchor;
    }
    prov_hl_state_t st = hl_cache_start(start);

    /* scan start..vis_first to reach the viewport state (filling the cache) */
    for (proven_size_t L = start; L < vis_first; L++) {
        proven_size_t n = hl_read_line(b, L, lc, g_hl_line);
        st = prov_hl_line(lang, st, g_hl_line, n, &s->theme, g_hl_scratch);
        if (have_cache) { g_hl_cache.ends[L] = st; if (L + 1 > g_hl_cache.n_valid) g_hl_cache.n_valid = L + 1; }
    }

    /* color the visible lines into g_hl_fg (document-byte indexed from base) */
    proven_u8 fgline[HL_LINECAP];
    for (proven_size_t L = vis_first; L < vis_last; L++) {
        proven_size_t n = hl_read_line(b, L, lc, g_hl_line);
        st = prov_hl_line(lang, st, g_hl_line, n, &s->theme, fgline);
        if (have_cache) { g_hl_cache.ends[L] = st; if (L + 1 > g_hl_cache.n_valid) g_hl_cache.n_valid = L + 1; }
        proven_size_t off = prov_buffer_line_start(b, L) - base;
        for (proven_size_t i = 0; i < n && off + i < span; i++) g_hl_fg[off + i] = fgline[i];
    }

    *fg_base = base; *fg_len = span;
    return g_hl_fg;
}
...
static void render_panes(prov_session_t *s, int node, prov_rect_t a,
    ...
    if (nd->kind == PROV_PANE_LEAF) {
        ...
        if (ch > 0 && cwc > 0 && (proven_size_t)ch * (proven_size_t)cwc <= (proven_size_t)tmpcap) {
            ...
            proven_size_t fgb = 0, fgl = 0;
            const proven_u8 *fgmap = build_highlight(s, ed, nd->top, (proven_size_t)ch, &fgb, &fgl);
            prov_render_into_hl(ed, nd->top, (proven_size_t)ch, (proven_size_t)cwc,
                                  s->cfg.tabstop, tmp,
...
static void build_cmdline(prov_session_t *s, char *cmdline, proven_size_t cap) {
    ...
    if (s->panel_open) {
        /* Unified: the panel's key legend (and any active text-input echo) is shown
         * here on the global command line — the panel box no longer carries its own
         * footer. Shortcut keys are wrapped in \x01..\x01 (present() renders those
         * bold+underline). `K` wraps a key; `B` is the packed separator. */
        #define K(x) "\x01" x "\x01"
        #define B    "\xe2\x94\x82"
        if (s->panel_verb)
            CL("  {} press a slot key " K("a-z") " / " K("0-9") "  (" K("Esc") " cancel)",
               PROVEN_ARG(prov_cstr_view(panel_verb_prompt(s->panel_verb))));
        else if (s->panel_filter)
            CL("  find: {}_   (" K("Enter") " keep \xc2\xb7 " K("Esc") " clear)", PROVEN_ARG(prov_cstr_view(s->panel.filter)));
            ...
            CL("  types: {}\xe2\x96\x8f{}   (" K("Enter") " done " B " ; separates suffixes)",
               PROVEN_ARG(prov_cstr_view(head)), PROVEN_ARG(prov_cstr_view(tail)));
        }
        else if (s->panel_kind == PANEL_K_HELP)
            CL("  " K("h") "+key:page" B K("ikjl") ":scroll" B K("Space") ":overview" B K("w") ":move" B K("q") ":close");
        else if (s->panel_help)
            CL("  " K("ikjl") ":scroll" B K("w") ":move" B K("Esc") ":back");
        else if (s->panel_kind == PANEL_K_HEXEDIT)
            CL("  edit text" B K("Shift+arrows") ":select" B K("^A^C^X^V") B K("^Z^Y") ":undo" B K("^S") ":write" B K("Esc") ":cancel");
        else if (s->panel_kind == PANEL_K_FIND)
            CL("  " K("n") "/" K("N") ":next/prev" B K("r") ":replace" B K("a") ":all" B K("x") ":rgx" B K("w") ":word" B K("c") ":case" B "{}" K("Tab") ":field" B K("Esc") ":close",
               PROVEN_ARG(prov_cstr_view(s->find.has_scope ? K("s") ":sel" B : "")));
        else if (s->panel_kind == PANEL_K_SAVEAS) {
            if (s->saveas_state == SA_CONFIRM)
                CL("  file exists \xe2\x80\x94 " K("y") ":overwrite" B K("n") ":rename" B K("Esc") ":cancel");
            else
                CL("  " K("Enter") ":save" B K("Esc") ":cancel" B "type the file path");
        }
        else if (s->panel_kind == PANEL_K_BROWSER && s->browse.save)
            CL("  " K("Enter") ":write" B K("o") ":write" B K("K") ":enter" B K("I") ":up" B K("J") "/" K("L") ":back/fwd" B K("d") ":reset" B K("t") ":sort" B K("f") ":types" B K("e") ":enc" B K("c") ":cancel");
        else if (s->panel_kind == PANEL_K_BROWSER)
            CL("  " K("ikjl") ":move" B K("o") ":open" B K("K") ":enter" B K("I") ":up" B K("J") "/" K("L") ":back/fwd" B K("t") ":sort" B K("f") ":types" B K("e") ":enc" B K("x") ":hex" B K("v") ":preview");
        else   /* generic list panel */
            CL("  " K("o") ":ok" B K("c") ":cancel" B K("d") ":discard" B K("0g") ":last" B K("3g") ":3rd item" B K("ss") ":find");
        #undef K
        #undef B
        ...
        for (int i = 0; i < pbi; i++) ph[i] = pv[i];
        { int j = 0; for (int i = pbi; pv[i]; i++) pt[j++] = pv[i]; }
        CL("  {}: {}\xe2\x96\x8f{}", PROVEN_ARG(prov_cstr_view(s->prompt_label)),
        ...
        char hint[120];
        if (s->zx_pending[0]) {
            /* in-progress command: show "<typed>:<label>" bright+bold (\x01), then the
             * sub-keys. describe returns "label: k:key k:key …" — split at the first
             * ": " so "<label>" merges with the typed prefix as "z:meta". */
            prov_cmd_describe(&s->parser, hint, sizeof hint);
            const char *sep = hint;
            while (*sep && !(sep[0] == ':' && sep[1] == ' ')) sep++;
            if (*sep) {                                   /* "label: subkeys" */
                char label[48];
                proven_size_t ll = (proven_size_t)(sep - hint);
                if (ll >= sizeof label) ll = sizeof label - 1;
                for (proven_size_t i = 0; i < ll; i++) label[i] = hint[i];
                label[ll] = '\0';
                CL("  \x01{}:{}\x01 {}", PROVEN_ARG(prov_cstr_view(s->zx_pending)),
                   PROVEN_ARG(prov_cstr_view(label)), PROVEN_ARG(prov_cstr_view(sep + 2)));
            } else {                                      /* no label: just the typed prefix */
                CL("  \x01{}\x01 {}", PROVEN_ARG(prov_cstr_view(s->zx_pending)),
...
static void handle_panel_mouse(prov_session_t *s, prov_key_t k) {
    ...
    int br = r0 + h - 1;                          /* bottom border (mirrors draw_panel) */
    int ctop = r0 + 1, ch = (br - 1) - ctop + 1; if (ch < 1) ch = 1;   /* content rows */
    ...
    if (k.mbtn == PROV_MB_LEFT && k.mact == PROV_ME_PRESS && list_panel) {
        ...
        if (s->panel_kind == PANEL_K_BROWSER) {
            ...
            int bodyh = (br - 3) - ctop;                     /* mirrors draw_panel: list (+preview) area */
            if (bodyh < 1) bodyh = 1;
            int list_h = bodyh;
            if (s->browse.preview && bodyh >= 6) { list_h = bodyh * 3 / 5; if (list_h < 2) list_h = 2; }
            last = ctop + list_h - 1;
        }
...
int main(int argc, char **argv) {
    ...
    /* Resolve the color theme (RFC-0021): read theme= + any *.theme.ini beside the
     * config. Loaded into the session now; nothing renders color until RFC-0022. */
    {
        char cdir[1100];
        s.theme = prov_theme_resolve(a, config_dir(a, cdir, sizeof cdir) ? cdir : NULL, s.cfg.theme);
    }
    ...
    while (s.running) {
        ...
        /* F1: context-aware help. In a panel -> that panel's own help page; in ed
         * mode -> the ed (modeless) help; in field mode (a / [N]a) -> the field
         * page; otherwise -> the zx keyboard help. */
        if (k.kind == PROV_KEY_F1) {
            if (s.panel_open) {
                if (s.panel_kind != PANEL_K_HELP) { s.panel_help = true; s.panel_help_scroll = 0; }
            } else if (s.hex_pending == HEX_PEND_NONE && s.prompt_kind == PROMPT_NONE
                       && !buf_is_binary(&s, active_buf(&s))) {
                int topic = s.mode == MODE_ED    ? 'E'
                          : s.mode == MODE_FIELD ? 'a'
                          : 0;
                panel_open_help(&s, topic);
            }
            continue;
        }
