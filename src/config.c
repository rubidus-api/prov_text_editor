...
prov_config_t prov_config_default(void) {
    ...
    c.mouse = true;
    c.theme[0] = '\0';              /* "" = prov-dark (RFC-0021) */
    return c;
}

static bool is_space(char c) { return c == ' ' || c == '\t' || c == '\r'; }
/* INI naming rule (prov convention): section names and key names must START with a
 * lowercase ASCII letter and otherwise use only lowercase letters, digits and
 * underscore — i.e. [a-z][a-z0-9_]*. No uppercase, no spaces, no hyphen, no
 * leading digit/underscore. Case-stable, identifier-like, portable. */
static bool is_lower(char c) { return c >= 'a' && c <= 'z'; }
static bool is_key_ch(char c) {
    return is_lower(c) || (c >= '0' && c <= '9') || c == '_';
}
/* validate a whole name [s,e): non-empty, lowercase-letter start, is_key_ch rest. */
static bool name_ok(const proven_u8 *s, proven_size_t n) {
    if (n == 0 || !is_lower((char)s[0])) return false;
    for (proven_size_t i = 1; i < n; i++) if (!is_key_ch((char)s[i])) return false;
    return true;
}
...
static void apply(prov_config_t *c, proven_u8str_view_t sec, proven_u8str_view_t key,
    ...
    if (proven_u8str_view_eq(sec, PROVEN_LIT("editor"))) {
        if (keyeq(key, "tabstop") && vt == V_INT && iv >= 1 && iv <= 64) {
            ...
            c->mouse = bv;
        } else if (keyeq(key, "theme") && vt == V_STR) {
            prov_cstr_set(c->theme, sizeof c->theme, sv);   /* RFC-0021 color theme name */
        }
...
prov_config_result_t prov_config_parse(prov_config_t *cfg, proven_allocator_t alloc,
    ...
    while (i < len) {
        ...
        if (at(s, a) == '[') {                         /* [section] */
            ...
            if (!name_ok(s.ptr + a, b - a)) {          /* [a-z][a-z0-9_]* */
                r.ok = false; r.line = line;
                r.message = "section name must be [a-z][a-z0-9_]*";
                return r;
            }
        ...
        if (!name_ok(s.ptr + k0, a - k0)) {            /* [a-z][a-z0-9_]* */
            r.ok = false; r.line = line; r.message = "key name must be [a-z][a-z0-9_]*"; return r;
        }
...
const char *prov_config_default_text(void) {
    return
        ...
        "# Section names and key names are [a-z][a-z0-9_]* : start with a lowercase\n"
        "# letter, then lowercase letters / digits / underscore (no uppercase,\n"
        "# spaces, or hyphens).\n"
        ...
        "\n"
        "[ui]\n"
        "theme = \"prov_dark\"  # color theme name (built-in: prov_dark/prov_light/mono,\n"
        "                     #  or a [theme-NAME] from a *.theme.ini beside this file).\n"
        "                     #  Names use lowercase letters, digits and underscore.\n"
        "\n"
