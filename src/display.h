...
typedef struct {
    ...
    proven_u8  fg;        /* foreground color, BIASED: 0 = terminal default,
                           * 1..16 = ANSI color (index-1). Zero-init = default, so
                           * chrome needs no changes; only the syntax highlighter
                           * (RFC-0021/0022) sets it. PROV_CELL_FG(n) builds it. */
} prov_cell_t;

/* Build a biased cell fg from an ANSI index 0..15 (or -1 = default -> 0). */
#define PROV_CELL_FG(ansi) ((proven_u8)((ansi) < 0 ? 0 : (ansi) + 1))
...
/* Optional syntax-highlight foreground source: `fgmap[byte - fg_base]` is the
 * biased cell fg (0 = default) for each document byte in [fg_base, fg_base+fg_len).
 * Pass fgmap=NULL for no highlighting (chrome / unhighlighted buffers). */
