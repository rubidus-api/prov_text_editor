...
struct prov_buffer {
    ...
    proven_size_t revision;       /* monotonic edit counter (bumped by every raw
                                   * insert/delete, incl. undo/redo); a cache key
                                   * for the syntax highlighter (RFC-0022) */
};
...
static proven_err_t insert_raw(prov_buffer_t *b, proven_size_t at,
    ...
    b->revision++;                 /* content about to change (RFC-0022 cache key) */
...
proven_size_t prov_buffer_revision(const prov_buffer_t *buf) {
    return buf->revision;
}
