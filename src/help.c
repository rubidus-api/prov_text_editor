...
static const char *P_APPEND[] = {    /* 'a' */
    "a / [N]a  — field mode: a bounded, underlined input region.",
    "",
    "  • Type a fragment, then Esc.  [N]a stamps it N times:  80a- => 80 dashes.",
    "  • Insert-only; move/select/edit freely *within* the region, not outside.",
    "  • Ctrl+X/C/V clipboard · Ctrl+Z/Y undo (this session only) · Ctrl+A select.",
    "  • on/op open N filled lines; c{motion} changes a target the same way.",
    "  • The whole session commits as one undo step.",
};
...
static const char *P_META[] = {      /* 'z' */
    "z   — meta namespace.",
    "",
    "  zx return to Ed mode    zy redo    zw write    za write-as    zq quit",
