...
static bool link_exe(const char *out, const Nob_File_Paths *a,
    ...
    if (nob_needs_rebuild(out, all.items, all.count) != 0) {
        ...
        /* Windows: link statically so the .exe is self-contained (no
         * libwinpthread-1.dll / libgcc dependency at runtime). */
        if (g_win64) nob_cmd_append(&cmd, "-static", "-static-libgcc", "-lwinpthread");
